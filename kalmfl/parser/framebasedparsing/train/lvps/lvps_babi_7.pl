...
lvp(Lexeme,'','Hold',verb,[pair('Person',index(2,5),[nsubj],required),pair('Theme',index(2,1),[obj,amod,advmod],required)]) :- fn_synonym('carry','Hold',Lexeme).
...
lvp(Lexeme,'','Giving',verb,[pair('Owner',index(5,2),[nsubj],required),pair('Theme',index(5,4),[obj],required),pair('Recipient',index(5,7),[obl_to],optional)]) :- fn_synonym('give','Giving',Lexeme).
