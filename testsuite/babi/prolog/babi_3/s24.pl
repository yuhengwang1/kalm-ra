:- multifile frame/2.

happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),1).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',football)]),2).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),3).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),4).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),5).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),6).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',hallway)]),7).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),8).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',garden)]),9).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bedroom)]),10).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',football)]),11).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',garden)]),12).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',milk)]),13).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',football)]),14).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',milk)]),15).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',milk)]),16).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),17).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',football)]),18).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',milk)]),19).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',kitchen)]),20).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',hallway)]),21).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',garden)]),22).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),23).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bathroom)]),24).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),25).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),26).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),27).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),28).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',apple)]),29).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),30).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),31).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bedroom)]),32).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),33).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),34).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),35).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),36).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bedroom)]),37).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',kitchen)]),38).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',milk)]),39).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',apple)]),40).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),41).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',milk)]),42).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),43).
happensAt(frame('Travel',[fe('Person',john),fe('Place',office)]),44).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',garden)]),45).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',milk)]),46).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',football)]),47).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',kitchen)]),48).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),49).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',milk)]),50).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),51).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),52).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),53).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',football)]),54).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',apple)]),55).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',apple)]),56).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),57).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),58).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),59).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',milk)]),60).
q(61,Answer) :- questionedAt(frame('Located',[fe('Entity',milk),fe('Place',Answer),fe('Before_place',bathroom)]),61).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',kitchen)]),62).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),63).
q(64,Answer) :- questionedAt(frame('Located',[fe('Entity',milk),fe('Place',Answer),fe('Before_place',bathroom)]),64).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',apple)]),65).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',football)]),66).
q(67,Answer) :- questionedAt(frame('Located',[fe('Entity',milk),fe('Place',Answer),fe('Before_place',bathroom)]),67).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',milk)]),68).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',football)]),69).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),70).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',apple)]),71).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),72).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',apple)]),73).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),74).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',milk)]),75).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',kitchen)]),76).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',milk)]),77).
happensAt(frame('Travel',[fe('Person',john),fe('Place',office)]),78).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),79).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),80).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),81).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',apple)]),82).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',apple)]),83).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),84).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),85).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),86).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),87).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bedroom)]),88).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',hallway)]),89).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',garden)]),90).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',milk)]),91).
happensAt(frame('Travel',[fe('Person',john),fe('Place',office)]),92).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),93).
q(94,Answer) :- questionedAt(frame('Located',[fe('Entity',football),fe('Place',Answer),fe('Before_place',office)]),94).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),95).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bedroom)]),96).
q(97,Answer) :- questionedAt(frame('Located',[fe('Entity',football),fe('Place',Answer),fe('Before_place',office)]),97).
