:- multifile frame/2.

happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),1).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',football)]),2).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',apple)]),3).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',office)]),4).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',hallway)]),5).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',football)]),6).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',apple)]),7).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bathroom)]),8).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',office)]),9).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',football)]),10).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',hallway)]),11).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),12).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),13).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),14).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),15).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',apple)]),16).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',football)]),17).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',apple)]),18).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),19).
happensAt(frame('Obtain',[fe('Person',mary),fe('Theme',football)]),20).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',apple)]),21).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',garden)]),22).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',apple)]),23).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',garden)]),24).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',garden)]),25).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),26).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bedroom)]),27).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),28).
happensAt(frame('Discard',[fe('Person',mary),fe('Theme',football)]),29).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),30).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),31).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),32).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),33).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),34).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),35).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bedroom)]),36).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),37).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',garden)]),38).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),39).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),40).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),41).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),42).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',milk)]),43).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',milk)]),44).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),45).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),46).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),47).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',milk)]),48).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bedroom)]),49).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),50).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',garden)]),51).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',office)]),52).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),53).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),54).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),55).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',milk)]),56).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',milk)]),57).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),58).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),59).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',kitchen)]),60).
happensAt(frame('Travel',[fe('Person',john),fe('Place',office)]),61).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bathroom)]),62).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),63).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),64).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',football)]),65).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),66).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',football)]),67).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),68).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),69).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',office)]),70).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',milk)]),71).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),72).
q(73,Answer) :- questionedAt(frame('Located',[fe('Entity',milk),fe('Place',Answer),fe('Before_place',hallway)]),73).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),74).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),75).
q(76,Answer) :- questionedAt(frame('Located',[fe('Entity',milk),fe('Place',Answer),fe('Before_place',hallway)]),76).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',hallway)]),77).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),78).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),79).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bedroom)]),80).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',kitchen)]),81).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),82).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),83).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),84).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),85).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),86).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',football)]),87).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),88).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',football)]),89).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bathroom)]),90).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),91).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bedroom)]),92).
happensAt(frame('Obtain',[fe('Person',sandra),fe('Theme',football)]),93).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',hallway)]),94).
happensAt(frame('Discard',[fe('Person',sandra),fe('Theme',football)]),95).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),96).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',kitchen)]),97).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',bathroom)]),98).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',garden)]),99).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bedroom)]),100).
happensAt(frame('Travel',[fe('Person',john),fe('Place',office)]),101).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',office)]),102).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),103).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',apple)]),104).
happensAt(frame('Travel',[fe('Person',john),fe('Place',kitchen)]),105).
happensAt(frame('Travel',[fe('Person',john),fe('Place',garden)]),106).
q(107,Answer) :- questionedAt(frame('Located',[fe('Entity',apple),fe('Place',Answer),fe('Before_place',garden)]),107).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',office)]),108).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),109).
q(110,Answer) :- questionedAt(frame('Located',[fe('Entity',apple),fe('Place',Answer),fe('Before_place',garden)]),110).
happensAt(frame('Travel',[fe('Person',john),fe('Place',bathroom)]),111).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',kitchen)]),112).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',apple)]),113).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',apple)]),114).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',apple)]),115).
happensAt(frame('Travel',[fe('Person',john),fe('Place',hallway)]),116).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),117).
happensAt(frame('Obtain',[fe('Person',daniel),fe('Theme',apple)]),118).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',milk)]),119).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',football)]),120).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',garden)]),121).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',office)]),122).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),123).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',milk)]),124).
happensAt(frame('Travel',[fe('Person',sandra),fe('Place',hallway)]),125).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',hallway)]),126).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',bathroom)]),127).
happensAt(frame('Obtain',[fe('Person',john),fe('Theme',milk)]),128).
happensAt(frame('Discard',[fe('Person',john),fe('Theme',football)]),129).
happensAt(frame('Travel',[fe('Person',mary),fe('Place',bathroom)]),130).
happensAt(frame('Travel',[fe('Person',daniel),fe('Place',garden)]),131).
happensAt(frame('Discard',[fe('Person',daniel),fe('Theme',apple)]),132).
q(133,Answer) :- questionedAt(frame('Located',[fe('Entity',apple),fe('Place',Answer),fe('Before_place',bathroom)]),133).
